...
class MessageFlow {
    steps = new Map();
    activeFlows = new Map();
    async getChat(logger, message, contact) {
    ...
    async processFlow(logger, message, contact) {
        let currStepId = 1;
        const context = { logger, contact, message };
        while (true) {
            const step = this.getStep(currStepId, logger);
            const result = await this.executeStep(step, context);
            if (result.isFinal) {
                logger.log(`A etapa ${currStepId} retornou um chat com sucesso!`, result.chat);
