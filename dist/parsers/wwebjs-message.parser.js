...
class WWEBJSMessageParser {
    static async parse(logger, instance, message, skipParsingFile = false, skipParsingQuoted = false) {
        ...
        if (!skipParsingQuoted && message.hasQuotedMsg) {
            ...
            const quotedId = await WWEBJSMessageParser.getQuotedId(logger, message);
    ...
    static async getQuotedId(logger, message) {
