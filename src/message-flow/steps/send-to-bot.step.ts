import prismaService from "../../services/prisma.service";
import Step, { FinalStep, StepContext } from "./step";

interface SendToBotStepOptions {
	instance: string;
	stepId: number;
	sectorId: number;
}

export default class SendToBotStep implements Step {
	private readonly instance: string;
	private readonly sectorId: number;
	public readonly id: number;

	constructor({ instance, stepId, sectorId }: SendToBotStepOptions) {
		this.id = stepId;
		this.instance = instance;
		this.sectorId = sectorId;
	}

	public async run(ctx: StepContext): Promise<FinalStep> {
		ctx.logger.log("Enviando mensagem para o bot...");
		const chat = await prismaService.wppChat.create({
			data: {
				instance: this.instance,
				type: "RECEPTIVE",
				userId: -1,
				sectorId: this.sectorId,
				contactId: ctx.contact.id
			}
		});

		ctx.logger.log("Chat criado.", chat);

		return {
			isFinal: true,
			chat
		};
	}
}
