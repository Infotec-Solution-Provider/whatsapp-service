...
assert.equal(
	redactFrontendInstanceFromRoute("/api/instances/tenant-a/query", "tenant-a"),
...
for (const source of ["render_to_effect_sampled_1_in_20", "sampled_1_in_5_window_max", "sampled_1_in_10"]) {
...
assert.throws(
	() =>
		parseFrontendPerformanceBatch({
			...parsed,
			metrics: [{ name: "message.body", value: 1, unit: "count", occurredAt: now, route: "/x" }]
		}),
...
assert.deepEqual(
	runtimeMetrics.metrics.map(({ name, unit }) => ({ name, unit })),
	[
...
const diagnosticMetrics = parseFrontendPerformanceBatch({
	...parsed,
	startedAt: now,
	metrics: [
		{ name: "volume.chats_loaded", value: 2_000, unit: "count", occurredAt: now, route: "/tenant-a/chats" },
		{ name: "volume.messages_loaded", value: 25_000, unit: "count", occurredAt: now, route: "/tenant-a/chats" },
		{
			...
			occurredAt: now,
			route: "/tenant-a/chats",
			tags: { phase: "file_upload_chunk", outcome: "timeout" }
		},
		{ name: "file_send.bytes", value: 5_242_880, unit: "bytes", occurredAt: now, route: "/tenant-a/chats" },
		{ name: "file_send.chunks", value: 2, unit: "count", occurredAt: now, route: "/tenant-a/chats" },
		{
			name: "error.count",
			value: 1,
			unit: "count",
			...
			tags: {
				...
				errorMessage: "Mensagem privada da cliente Maria"
			}
		}
	]
});
assert.deepEqual(
	diagnosticMetrics.metrics.map(({ name, unit }) => ({ name, unit })),
	[
		{ name: "volume.chats_loaded", unit: "count" },
		{ name: "volume.messages_loaded", unit: "count" },
		{ name: "file_send.duration", unit: "ms" },
		{ name: "file_send.bytes", unit: "bytes" },
		{ name: "file_send.chunks", unit: "count" },
		{ name: "error.count", unit: "count" }
	]
);
assert.deepEqual(diagnosticMetrics.metrics[5]?.tags, {
	errorName: "AxiosError",
	errorFingerprint: "a1b2c3d4",
	errorCategory: "network",
	errorSource: "file_send",
