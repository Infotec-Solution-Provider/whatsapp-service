...
const ALLOWED_COMPONENTS = new Set([
	"AppProvider", "ChatHeader", "ChatMessagesList", "ChatProvider", "ChatSendMessageArea",
	"ChatsMenuFilters", "ChatsMenuList", "ContactsProvider", "CustomersProvider", "Header",
	"InternalChatProvider", "InternalGroupsProvider", "ReadyMessagesProvider",
	"RenderInternalChatMessages", "RenderInternalGroupMessages", "RenderWhatsappChatMessages",
	"RouteFeatureGate", "SocketProvider", "WhatsappProvider"
]);
const ALLOWED_INTERACTIONS = new Set([
	"chat_filter", "chat_filter_ready", "chat_sort", "chat_sort_ready", "open_chat",
	"open_chat_ready", "socket_event_ready"
]);
const ALLOWED_SOCKET_EVENTS = new Set([
	"internal_chat_finished", "internal_chat_started", "internal_message", "internal_message_delete",
	"internal_message_edit", "internal_message_status", "report_status", "telephony_call_received",
	"wpp_chat_finished", "wpp_chat_started", "wpp_chat_transfer", "wpp_contact_messages_read",
	"wpp_message", "wpp_message_delete", "wpp_message_edit", "wpp_message_reaction",
	"wpp_message_status", "wwebjs_auth", "wwebjs_qr", "wwebjs_session_status"
]);
const ALLOWED_SOURCES = new Set([
	"1s_window", "1s_window_max", "10s_window", "30s_window", "30s_window_average", "30s_window_max",
	"30s_window_total", "axios_default", "axios_instance", "committed",
	"direction", "fetch", "field", "frames_per_second", "internal", "normalized_per_minute",
	"render_to_effect_sampled_1_in_10", "render_to_effect_sampled_1_in_20",
	"sampled_1_in_5", "sampled_1_in_5_window_max", "sampled_1_in_10", "search", "type", "whatsapp",
	"window_reservoir"
]);
const ALLOWED_ERROR_NAMES = new Set([
	"AbortError", "AggregateError", "AxiosError", "DOMException", "Error", "EvalError",
	"NetworkError", "RangeError", "ReferenceError", "SyntaxError", "TypeError", "URIError"
]);
const ALLOWED_INITIATOR_TYPES = new Set([
	"audio", "beacon", "css", "fetch", "iframe", "img", "link", "navigation", "other",
	"script", "video", "xmlhttprequest"
]);
...
const SAFE_TELEMETRY_ROUTE_SEGMENTS = new Set([
	":id", ":instance", ":value", "api", "ai", "ai-agents", "ai-settings", "ai-supervisor", "audio",
	"agents", "audience", "auth", "auth-batches", "auto-response", "auto-response-rules", "beacon",
	"browser-resource", "channels", "chat", "chats", "cities", "clients", "config", "copy",
	"contact-requests", "contacts", "css", "customer", "customers", "dashboard", "dashboards", "details",
	"execute", "execute-report-sql", "export", "export-report-sql", "fetch", "files", "filters", "finish",
	"flows", "full", "funnel", "generated",
	"geo", "goals-dashboard", "iframe", "img", "instances", "internal", "internal-groups", "items", "knowledge",
	"lead-origin-quality", "link", "login", "logs", "lost-reasons", "mailing-analysis", "marketing",
	"layout", "mass-messages", "messages", "metric-tables", "metrics", "monitor", "navigation", "new", "next", "notification-preferences",
	"notifications", "operator-performance", "operators", "other", "parameters", "preview", "public", "qr",
	"query", "read", "ready-messages", "regua-carteira-sintetico-whatsapp", "report-generator",
	"reports", "reports-history", "reset-qr", "restart", "retry", "sales", "save", "script", "sector", "sectors",
	"session", "session-monitor", "sessions", "sip-config", "states", "stream", "supervisor-chat",
	"team-goals", "templates", "tenant-config", "tools", "usage", "users", "video", "wallets", "whatsapp",
	"whatsapp-senders", "xmlhttprequest"
]);
...
function normalizedEffectiveType(value: unknown): string | null {
	...
	return effectiveType && new Set(["slow-2g", "2g", "3g", "4g"]).has(effectiveType)
		? effectiveType
		: null;
}
...
export function redactFrontendInstanceFromRoute(value: unknown, instance: string): string {
	...
	const segments = path.split("/").map((segment) =>
		candidates.has(segment.toLowerCase()) ? ":instance" : segment
	);
...
export function parseFrontendPerformanceBatch(raw: unknown, trustedInstance?: string): FrontendPerformanceBatch {
	...
	const providedBatchId = input["batchId"] === undefined
		? null
		: boundedString(input["batchId"], 64, "batchId");
	if (providedBatchId && !/^([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}|legacy-[0-9a-f]{56})$/i.test(providedBatchId)) {
	...
	const hardwareConcurrency = optionalPositiveNumber(
	...
	const batchChecksum = createHash("sha256").update(canonicalJson({
		schemaVersion: 1,
		sessionId,
		buildId,
		startedAt,
...
function breakdownAccumulator(
	...
	const effectiveKey = metricGroupCount < FRONTEND_PERFORMANCE_SUMMARY_MAX_BREAKDOWN_GROUPS_PER_METRIC
		? key
		: fallbackKey;
...
class FrontendPerformanceService {
	...
	public async summary(instance: string, filters: FrontendPerformanceFilters) {
		...
		const approximate = truncated || breakdownsGroupedAsOther || [...metricValues.values()].some(({ summary }) =>
			summary.count > summary.values.length
		);
	...
	public async *exportCsvChunks(instance: string, filters: FrontendPerformanceFilters): AsyncGenerator<string> {
		const header = [
			"sample_id", "batch_id", "sample_index", "occurred_at", "session_id", "user_id", "build_id", "device_class", "browser",
		...
		const highWaterMark = snapshot?.id ?? 0n;
		const loadPage = () => prismaService.frontendPerformanceSample.findMany({
				where: this.paginatedSampleWhere(instance, filters, cursor, highWaterMark),
		...
		while (true) {
			...
			const rows = samples.map((sample) => [
				sample.id.toString(), sample.batchId, sample.sampleIndex, sample.occurredAt.toISOString(),
				sample.session.sessionId, sample.session.userId,
