...
export async function inspectWppSchema(connection: PoolConnection) {
	// LEFT avoids interpreting the underscore in wpp_ as a LIKE wildcard.
	const tables = await sql<RowDataPacket[]>(connection, `SELECT TABLE_NAME AS name,
	...
	const columns = await sql<RowDataPacket[]>(connection, `SELECT TABLE_NAME AS table_name,
	...
	const indexes = await sql<RowDataPacket[]>(connection, `SELECT TABLE_NAME AS table_name,
	...
	const references = await sql<RowDataPacket[]>(connection, `SELECT TABLE_SCHEMA AS source_schema,
	...
	const triggers = await sql<RowDataPacket[]>(connection, `SELECT TRIGGER_NAME AS name,
